...
class Gmail(GmailBaseModel):
    ...
    @staticmethod
    def list_messages(label_id: str = None, sender: str = None):
        ...
        try:
            ...
            messages = unread_message_list.dict().get("messages", [])
    ...
    def mark_all_read(label_id: str = None, sender: str = None):
        ...
        try:
            ...
            unread_messages = Gmail.list_messages(label_id, sender)
    ...
    def download_attachment(label_name: str = None, sender: str = None):
        ...
        try:
            ...
            if label_name and not sender:
                label_id: str = Gmail.list_labels().get(label_name)
                messages = Gmail.list_messages(label_id=label_id)
            elif sender and not label_name:
                messages = Gmail.list_messages(sender=sender)
                for msg in messages:
    ...
    def delete_filter(filter_id: str):
        ...
        """
        try:
            service().users().settings().filters().delete(userId="me", id=filter_id)
